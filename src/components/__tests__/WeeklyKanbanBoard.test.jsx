...
describe('WeeklyKanbanBoard', () => {
  ...
  it('loads weekly tasks on initial render', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Sprint planning & retrospective')).toBeInTheDocument()
    })
  ...
  it('all weekly tasks start in the Todo column', async () => {
    ...
    const weeklyTodoColumn = todoElements.find(element => 
      element.closest('.weekly-kanban-board')
    )?.closest('.column')
    
    if (weeklyTodoColumn) {
      expect(weeklyTodoColumn).toContainElement(screen.getByText('Sprint planning & retrospective'))
      expect(weeklyTodoColumn).toContainElement(screen.getByText('Monthly goals review'))
      expect(weeklyTodoColumn).toContainElement(screen.getByText('Team building activity'))
    }
  })
